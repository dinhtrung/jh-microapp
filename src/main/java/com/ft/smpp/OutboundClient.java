...
public class OutboundClient extends Client {
	...
	private EcasSmppClient clientBootstrap;
	...
	public OutboundClient() {
		...
		clientBootstrap = new EcasSmppClient();
	}
...
//			SmsMtConfig.activeSessions.set(smppSession, 1);
			...
			logger.info(this.getConfiguration().getName());
			try {
				DefaultSmppSession channel = clientBootstrap.doOpen(config, sessionHandler);
				channel.unbind(10000);
			} catch(Exception e1) {
				logger.error("Cannot send UNBIND also");
				e1.printStackTrace();
			}
...
//				SmsMtConfig.activeSessions.remove(smppSession);
				smppSession.destroy();
				smppSession = null;
	...
	public synchronized SubmitSmResp sendMessage(String shortcode, String msgData, String receiver) throws RecoverablePduException, UnrecoverablePduException, SmppTimeoutException, SmppChannelException, InterruptedException {
		// TODO Auto-generated method stub
		SubmitSm request = new SubmitSm();
//		request.getSourceAddress().setAddress(shortcode);
//		request.getDestAddress().setAddress(receiver);
		request.setSourceAddress(new Address(SmppConstants.TON_ALPHANUMERIC, SmppConstants.NPI_E164, shortcode));
		request.setDestAddress(new Address(SmppConstants.TON_INTERNATIONAL, SmppConstants.NPI_E164, receiver));
		...
		WindowFuture<Integer, PduRequest, PduResponse> future = this.getSession().sendRequestPdu(request, 10000, true);
		while (!future.isDone()){
			future.await();
		}
		return (SubmitSmResp) future.getResponse();
