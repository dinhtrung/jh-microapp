...
public class SmsMtConfig {
    ...
    @Autowired
    @Qualifier("smppProcessService")
    private SmppProcessService smppProcessor;

    public static LoadBalancedList<OutboundClient> balancedList = LoadBalancedLists
            .synchronizedList(new RoundRobinLoadBalancedList<OutboundClient>());

    public static LoadBalancedList<OutboundClient> chargingList = LoadBalancedLists
            .synchronizedList(new RoundRobinLoadBalancedList<OutboundClient>());

    @PostConstruct
    protected void initialize() {
        initializeSmppConn();
        initializeChargingConn();
    }
    ...
    private void initializeSmppConn() {
        ...
        for (SmppSessionConfiguration cfg : props.getSmppConfig()) {
            ...
            log.info("Add new session: " + conn.getSessionId() + " --- " + conn.getConfiguration().getHost() + " : "
                    + conn.getConfiguration().getPort());
            balancedList.set(conn, 1);
        }
    }

    private void initializeChargingConn() {
        //log.info("Trying to connect to our Charging Gateway");
        for (SmppSessionConfiguration cfg : props.getChargingConfig()) {
            OutboundClient conn = new OutboundClient();
            conn.initialize(cfg, new SmppClientMessageService(smppProcessor), props);
            conn.scheduleReconnect();
            log.info("Add new session: " + conn.getSessionId() + " --- " + conn.getConfiguration().getHost() + " : "
                    + conn.getConfiguration().getPort());
            chargingList.set(conn, 1);
        }
    }
    ...
    public SubmitSmResp chargeMessage(String source, String text, String destination) throws Exception {
        SubmitSm request = copySubmitSm(props.getChargingSm());
        request.getSourceAddress().setAddress(source);
        request.getDestAddress().setAddress(destination);
        request.setShortMessage(CharsetUtil.encode(text, CharsetUtil.CHARSET_GSM));
        try {
            return sendCharge(request);
        } catch (Exception e) {
            //log.error("Cannot send SMS: " + source + " --> " + destination + ": " + text, e);
            throw e;
        }
    ...
    public static SubmitSmResp sendCharge(SubmitSm request) throws Exception {
        while (true) {
            final OutboundClient next = chargingList.getNext();
            final SmppSession session = next.getSession();
            if (session != null && session.isBound()) {
                return session.submit(request, 10000);
            }
    ...
    protected void stopAll() throws Throwable {
        ...
        for (LoadBalancedList.Node<OutboundClient> node : chargingList.getValues()) {
            node.getValue().shutdown();
        }
    }
